...
class API:
    ...
    def find_executable(self):
        ...
        if not observerward_path:
            logger.warninging("ObserverWard executable not found. Set OBSERVERWARD_PATH in config.py or set it to system enviroment.", origin="thirdpart")
            return False
        self.ward_path = observerward_path
        # nuclei
        nuclei_path = conf.nuclei_path if conf.get("nuclei_path", "") != "" else which('nuclei')
        if not nuclei_path:
            logger.warninging("Nuclei executable not found. Set NUCLEI_PATH in config.py or set it to system enviroment.", origin="thirdpart")
            return False
    ...
    def check_installation(self):
        try:
            ...
            if "observer_ward v" in result.stdout or "observer_ward v" in result.stderr:
                ...
                logger.warninging(f"ObserverWard execution failed: No version identifier found", origin="thirdpart")
                return False
            # nuclei
            result = subprocess.run(
            ...
            if "Nuclei Engine Version" in result.stdout or "Nuclei Engine Version" in result.stderr:
                ...
                logger.warninging(f"Nuclei execution failed: No version identifier found", origin="thirdpart")
                return False
            # fingerprints
            ...
            if "windows" in system:
                ...
                logger.warninging(f"Unsupported operating system: {system}", origin="thirdpart")
                return False
            if not file_path.exists():
                logger.warninging(f"Fingerprint location not found, try running: observerward --update-fingerprint", origin="thirdpart")
                return False
            ...
            if not dir_path.exists() and dir_path.is_dir():
                logger.warninging(f"ObserverWard-Nuclei plugin location not found, try running: observerward --update-plugin", origin="thirdpart")
                return False
