            ...
            for _ in files:
                ...
                try:
                    ...
                    if plugin_key not in KB["registered"]:
                        logger.warninging(f"Plugin {mod.name} hadn't been loaded. Skip.")
                        continue
                    # 执行禁用
            ...
            for _ in files:
                ...
                try:
                    ...
                    if plugin_key in KB["registered"]:
                        logger.warninging(f"Plugin {mod.name} had been enabled. Skip.")
                        continue
...
class BackgroundServer:
    ...
    def start(self):
        if self.running:
            logger.warninging("Web server is already running")
            return self
        self.running = True
