...
def _list():
    ...
    if "registered" not in KB or not KB["registered"]:
        logger.warninging("No scanners loaded.")
        return
    table = PrettyTable()
...
def initPlugins():
    ...
    for root, dirs, files in os.walk(path.dicts):
        ...
        for _ in files:
            ...
            try:
                ...
                logger.warninging(f'Error loading list {file}: {str(e)}')
    # 优先加载loader
    ...
    if not require_reverse_list == []:
        logger.warninging(f'Skip Scanners (Require Reverse): {colors.y}{require_reverse_list}{colors.e}')
    if not require_risk_list == []:
        logger.warninging(f'Skip Scanners (Require Risk): {colors.y}{require_risk_list}{colors.e}')
    ...
    if v == "crawler":
        ...
        if conf.enable_crawler is True:
            ...
            if not crawlergo_path:
                logger.warninging("Crawlergo executable not found. Set CRAWLERGO_PATH in config.py or set it to system enviroment.", origin="crawler")
