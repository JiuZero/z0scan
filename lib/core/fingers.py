...
class waf:
    ...
    def detector(self):
        ...
        try:
            ...
            for _id in range(1, int(Wcount()),1):
                try:
                    ...
                    if name:
                        logger.warninging("{}{}{} Protected by {}".format(colors.m, self.hostname, colors.e, name))
                        cv = {"hostname": self.hostname, 
                              "waf": name}
                        insertdb("info", cv)
                        return name
            ...
            if r.status_code in (493, 405, 403, 400) or r.status_code >= 500:
                logger.warninging("{}{}{} Abnormal response ({}), possible WAF detected".format(colors.m, self.hostname, colors.e, r.status_code))
                cv = {"hostname": self.hostname, 
                      "waf": "UNKNOW"}
                insertdb("info", cv)
                return "UNKNOW"
        ...
        # 3. 超时与连接问题很可能产生于WAF
            ...
            logger.warninging("{}{}{} An error occurred during the request, possible WAF detected".format(colors.m, self.hostname, colors.e))
            cv = {"hostname": self.hostname,
                  "waf": "UNKNOW"}
            insertdb("info", cv)
            return "UNKNOW"
