...
def decode(query):
    ...
    try:
        ...
        logger.warninging("Decode ldap error:{} sourquery:{}".format(ex, query))
    return info
...
def getldappath(buff):
    ...
    try:
        ...
        logger.warninging("Error:{}".format(ex), origin="LDAP")
        return None
...
def ldap_response(client, address):
    try:
        ...
        logger.warninging('Run ldap error:{} address:{}'.format(ex, address))
    finally:
        client.close()
